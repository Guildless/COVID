	...
	if (Equipment.EquippedItems.Contains(itemSlot) && ModularMeshes.Contains(itemSlot))
	{
		...
		if (GetEquippedItemSlot(itemSlot, invItem))
		{
			...
			if (internalArmor)
			{
				...
				Internal_OnArmorUnequipped(itemSlot);				
				OnEquipmentChanged.Broadcast(Equipment);
			}
		}
...
void UACFEquipmentComponent::EquipArmor_Implementation(AACFArmor* ArmorToEquip)
{
	...
	if (GetInventoryItem(ArmorToEquip, outItem) && CharacterOwner)
	{
		...
		if (ArmorToEquip->GetMeshComponent() && ArmorToEquip->GetMeshComponent()->SkeletalMesh) {
...
void UACFEquipmentComponent::AddSkeletalMeshComponent_Implementation(const UClass* ArmorClass)
{
	if (!ArmorClass) {
		return;
	}
	...
	if (ArmorToAdd && GetModularMesh(ArmorToAdd->GetItemSlot(), outMesh) && outMesh.meshComp)
	{		
...
bool UACFEquipmentComponent::AddSkeletalMeshComponent_Validate(const UClass* ArmorClass)
{
	return true;
}
