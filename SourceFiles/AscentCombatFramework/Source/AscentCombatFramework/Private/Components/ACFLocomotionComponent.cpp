...
void UACFLocomotionComponent::TickComponent(float DeltaTime, enum ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
...
void UACFLocomotionComponent::UpdateAimOffset(float deltatime)
{
	...
	aimOffest.AimOffsetYaw = FMath::FInterpTo(aimOffest.AimOffsetYaw, yawtarget, deltatime, AimOffsetSmoothingFactor);
	aimOffest.AimOffsetPitch = FMath::FInterpTo(aimOffest.AimOffsetPitch, pitchtarget, deltatime, AimOffsetSmoothingFactor);
}
	...
	if (GetOwner() && MovementComp && !MovementComp->IsFalling() && reproductionType == EMontageReproductionType::ERootMotion)
	{
		if (MovementComp->GetGroundMovementMode() == MOVE_Walking) {
			for (int i = 0; i < LocomotionStates.Num() - 1; i++)
			{
				...
				else if (LocomotionStates[i + 1].State != currentLocomotionState &&
					Speed > LocomotionStates[i].MaxStateSpeed + 5.f &&
					Speed <= LocomotionStates[i + 1].MaxStateSpeed + 5.f){
			...
			if (currentLocomotionState == ELocomotionState::ESprint)
			{
				...
				if (animInst) {
					float Direction = animInst->GetDirection();
