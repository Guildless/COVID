...
TArray<TEnumAsByte<ECollisionChannel>> AACFCharacter::GetEnemiesCollisionChannel() const {
  ...
  if (UACFFunctionLibrary::GetBattleType(GetWorld()) ==
    ...
    channels = UACFFunctionLibrary::GetACFTeamManager(GetWorld())->GetAllCollisionChannels(true);
  } else {
    channels = UACFFunctionLibrary::GetACFTeamManager(GetWorld())->GetEnemyCollisionChannels(CombatTeam);
  }
  return channels;
}
