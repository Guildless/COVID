...
void AACFAIController::SetCombatTeam_Implementation(const ETeam& newTeam)
{
	SetGenericTeamId(FGenericTeamId(uint8(newTeam)));
}

bool AACFAIController::SetCombatTeam_Validate(const ETeam& newTeam)
{
	return true;
}
...
void AACFAIController::SetGroupOwner(class UACFGroupAIComponent* _Group, int _groupIndex, bool _disablePerception, bool bOverrideTeam)
{
	if (_Group)
	{
		...
		if(bOverrideTeam)
		{
			SetGenericTeamId(FGenericTeamId(uint8(mGroup->CombatTeam)));
		}
	}
}
...
void AACFAIController::SetTarget(AActor* inTarget)
{
	...
	{	
		...
		if (entity && inTarget &&  IACFEntityInterface::Execute_IsEntityAlive(inTarget) && 
			UACFFunctionLibrary::AreEnemyTeams(GetWorld(), CombatTeam, IACFEntityInterface::Execute_GetEntityCombatTeam(inTarget))
			)
		{
