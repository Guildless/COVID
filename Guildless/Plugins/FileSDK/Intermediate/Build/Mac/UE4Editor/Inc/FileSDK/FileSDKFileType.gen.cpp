// Copyright Epic Games, Inc. All Rights Reserved.
/*===========================================================================
	Generated code exported from UnrealHeaderTool.
	DO NOT modify this manually! Edit the corresponding .h files instead!
===========================================================================*/

#include "UObject/GeneratedCppIncludes.h"
#include "FileSDK/Public/FileSDKFileType.h"
#ifdef _MSC_VER
#pragma warning (push)
#pragma warning (disable : 4883)
#endif
PRAGMA_DISABLE_DEPRECATION_WARNINGS
void EmptyLinkFunctionForGeneratedCodeFileSDKFileType() {}
// Cross Module References
	FILESDK_API UEnum* Z_Construct_UEnum_FileSDK_EFileSDKFileType();
	UPackage* Z_Construct_UPackage__Script_FileSDK();
// End Cross Module References
	static UEnum* EFileSDKFileType_StaticEnum()
	{
		static UEnum* Singleton = nullptr;
		if (!Singleton)
		{
			Singleton = GetStaticEnum(Z_Construct_UEnum_FileSDK_EFileSDKFileType, Z_Construct_UPackage__Script_FileSDK(), TEXT("EFileSDKFileType"));
		}
		return Singleton;
	}
	template<> FILESDK_API UEnum* StaticEnum<EFileSDKFileType>()
	{
		return EFileSDKFileType_StaticEnum();
	}
	static FCompiledInDeferEnum Z_CompiledInDeferEnum_UEnum_EFileSDKFileType(EFileSDKFileType_StaticEnum, TEXT("/Script/FileSDK"), TEXT("EFileSDKFileType"), false, nullptr, nullptr);
	uint32 Get_Z_Construct_UEnum_FileSDK_EFileSDKFileType_Hash() { return 2659028098U; }
	UEnum* Z_Construct_UEnum_FileSDK_EFileSDKFileType()
	{
#if WITH_HOT_RELOAD
		UPackage* Outer = Z_Construct_UPackage__Script_FileSDK();
		static UEnum* ReturnEnum = FindExistingEnumIfHotReloadOrDynamic(Outer, TEXT("EFileSDKFileType"), 0, Get_Z_Construct_UEnum_FileSDK_EFileSDKFileType_Hash(), false);
#else
		static UEnum* ReturnEnum = nullptr;
#endif // WITH_HOT_RELOAD
		if (!ReturnEnum)
		{
			static const UE4CodeGen_Private::FEnumeratorParam Enumerators[] = {
				{ "EFileSDKFileType::File", (int64)EFileSDKFileType::File },
				{ "EFileSDKFileType::Directory", (int64)EFileSDKFileType::Directory },
			};
#if WITH_METADATA
			const UE4CodeGen_Private::FMetaDataPairParam Enum_MetaDataParams[] = {
				{ "BlueprintType", "true" },
				{ "Directory.Name", "EFileSDKFileType::Directory" },
				{ "File.Name", "EFileSDKFileType::File" },
				{ "ModuleRelativePath", "Public/FileSDKFileType.h" },
			};
#endif
			static const UE4CodeGen_Private::FEnumParams EnumParams = {
				(UObject*(*)())Z_Construct_UPackage__Script_FileSDK,
				nullptr,
				"EFileSDKFileType",
				"EFileSDKFileType",
				Enumerators,
				UE_ARRAY_COUNT(Enumerators),
				RF_Public|RF_Transient|RF_MarkAsNative,
				UE4CodeGen_Private::EDynamicType::NotDynamic,
				(uint8)UEnum::ECppForm::EnumClass,
				METADATA_PARAMS(Enum_MetaDataParams, UE_ARRAY_COUNT(Enum_MetaDataParams))
			};
			UE4CodeGen_Private::ConstructUEnum(ReturnEnum, EnumParams);
		}
		return ReturnEnum;
	}
PRAGMA_ENABLE_DEPRECATION_WARNINGS
#ifdef _MSC_VER
#pragma warning (pop)
#endif
